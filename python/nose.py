...
def nose_bosphorus_sheet(folder):
...
def nose_region_ext(folder, noses, output):
	os.makedirs(output, exist_ok=True)
	
	for cloud in os.listdir(folder):
		if ".xyz" in cloud:
			input_cloud = folder + cloud
			nose_cloud = noses + cloud
			output_cloud = output + cloud
			
			cmd = ["../bin/nosecut", input_cloud, nose_cloud, output_cloud]
			subprocess.run(cmd)
			
			print(cloud, "nose extracted Ok!")

if __name__ == "__main__":
	folder = "../datasets/bosphorus/neutral/"
	noses = "../datasets/bosphorus/neutral/bsnose/"
	output = "../datasets/bosphorus-nose/neutral/"
